...
class ProductionConferenceServer:
    ...
    def _process_frame(self, frame_data, sender_addr):
        ...
        sender_ip, sender_port = sender_addr[:2]  # Extract IP and port
        ...
        # Update station registry with full endpoint info
        self._update_station(callsign, sender_addr)
        
        # Forward to all other stations
        forwarded = self._forward_frame(frame_data, callsign, sender_addr)
    ...
    def _update_station(self, callsign, sender_addr):
        """Update station registry with IP:port tracking"""
        sender_ip, sender_port = sender_addr[:2]  # Extract IP and port
        current_time = time.time()
        ...
        if callsign in self.stations:
            ...
            # Check for endpoint changes (IP or port)
            old_endpoint = f"{station_info['ip']}:{station_info['port']}"
            new_endpoint = f"{sender_ip}:{sender_port}"
            
            if old_endpoint != new_endpoint:
                print(f"📍 {callsign}: endpoint changed {old_endpoint} → {new_endpoint}")
                station_info['ip'] = sender_ip
                station_info['port'] = sender_port
        else:
            # New station
            print(f"🆕 NEW STATION: {callsign} at {sender_ip}:{sender_port}")
    ...
    def _forward_frame(self, frame_data, sender_callsign, sender_addr):
        """Forward frame with IP deduplication to eliminate duplicate packets"""
        sender_ip, sender_port = sender_addr[:2]
        sender_endpoint = f"{sender_ip}:{sender_port}"
        
        forwarded_count = 0
        failed_ips = set()
        
        # Collect unique target IPs (deduplication happens here)
        unique_target_ips = set()
        
        for callsign, info in self.stations.items():
            if callsign != sender_callsign:
                station_endpoint = f"{info['ip']}:{info['port']}"
                
                # Skip if same endpoint as sender (avoid echo)
                if station_endpoint == sender_endpoint:
                    continue
                
                # Add target IP to set (automatically deduplicates by IP)
                unique_target_ips.add(info['ip'])
        
        # Send one packet per unique IP address
        for target_ip in unique_target_ips:
            ...
            try:
                ...
                print(f"❌ Forward failed to {target_ip}: {e}")
                failed_ips.add(target_ip)
        
        # Remove stations at failed IPs
        if failed_ips:
            failed_stations = []
            for callsign, info in self.stations.items():
                if info['ip'] in failed_ips:
                    failed_stations.append(callsign)
            
            for callsign in failed_stations:
                station_info = self.stations[callsign]
                endpoint = f"{station_info['ip']}:{station_info['port']}"
                print(f"🗑️  Removing unreachable station: {callsign} at {endpoint}")
    ...
    def _cleanup_inactive_stations(self):
        ...
        for callsign, duration in inactive_stations:
            station_info = self.stations[callsign]
            endpoint = f"{station_info['ip']}:{station_info['port']}"
            print(f"⏰ TIMEOUT: {callsign} at {endpoint} - inactive for {duration/60:.1f} minutes")
            del self.stations[callsign]
    ...
    def _print_periodic_stats(self):
        ...
        if len(self.stations) > 0:
            ...
            for callsign, info in self.stations.items():
                ...
                endpoint = f"{info['ip']}:{info['port']}"
                print(f"   📡 {callsign} at {endpoint} ({info['frame_count']} frames, {age:.0f}s ago, timeout in {timeout_remaining/60:.1f}m)")
